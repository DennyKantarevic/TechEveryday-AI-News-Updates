...
export async function GET(request: NextRequest) {
  ...
  if (!token) {
    return htmlResponse("Invalid link", "The unsubscribe link is missing a token.", 400);
  }
  ...
  if (error || !subscription) {
    return htmlResponse("Invalid link", "This unsubscribe link is invalid or expired.", 404);
  }
  ...
  return htmlResponse(
    "Unsubscribed",
    "Daily email updates are turned off. You can re-enable them from Account settings."
  );
}
